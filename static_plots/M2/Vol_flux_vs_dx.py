...
for j in range(len(dx)):
  ...
  for i in range(len(exps1)):
     ...
     print '--------------------------------- \n'
     data1 = netCDF4.Dataset(path_to_file1).variables['TotalMassFlux'][:]/1.0E3 # Volume flux, m3/s
     #data2 = netCDF4.Dataset(path_to_file2).variables['TotalMassFlux'][:]/1.0E-3
  ...
  ax.errorbar(wind, OHT1_mean, OHT1_std, linestyle='-', marker='o', color=c, elinewidth=2, label=r'$\Delta$x = '+dx[j])

ax.legend(loc='upper left', fontsize=14, ncol=2)
ax.set_ylabel(r'Total melt water flux [m$^3$ s$^{-1}$]', fontsize=20)
