...
for j in range(len(dx)):
  ...
  # plot
  ax1.plot(Hi1_mean, tr1_mean, linestyle='', marker='o', color=colors[j], lw=2, label=r'$\Delta x$ = '+dx1[j])
  #ax2.plot(Hi2_mean, tr2_mean, linestyle='', marker='o', color=colors[j], lw=

  # fit line 
  xx=np.array(Hi1_mean)
  yy=np.array(tr1_mean)
  ...
  # TEXT
  ax1.text(1.3,tx[j],r'$TF$ = '+str(m)+' (r = '+str(r_value)+')', color=colors[j], fontsize=20)


ax1.legend(loc='upper left', fontsize=18, ncol=4)
ax1.set_xlabel(r'$H_{in}$ [TW]', fontsize=20)
ax1.set_ylabel(r'$\Phi_{in}$ [Sv]', fontsize=20)
#ax2.set_ylabel(r'$H_{in}$ [TW]', fontsize=20)
#ax1.set_ylabel(r'$T_{in}$ [Sv]', fontsize=20)

ax1.set_ylim(0,0.55)
#ax2.set_ylim(0,0.5)
ax1.set_xlim(-0.001,2.1)
#ax2.set_xlim(0.0,2.1)
