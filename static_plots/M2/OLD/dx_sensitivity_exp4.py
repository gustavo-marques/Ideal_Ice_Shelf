...
units = ['OHT(y=200 km) [TW]','OHT(y=460 km) [TW]','Sea ice volume [m$^3$]','Melt rate  [m yr$^{-1}$]','CDW transport at y = 480 km [Sv]','$T_{CDW}$ at y = 200 km [Sv]','Mean MLD at cont. shelf [m]','Ambient start. at cont. shelf [1/s]','Mean temperature at ML [C]','Mean salinity at ML [psu]','Total melt water flux [m$^3$ s$^{-1}$]']
...
for n in range(len(vars)):
   ...
   for i in range(len(labels1)):
       if var in var_list:
         path_file = path+fnames1[i]+'/Sigma_zstar/M2_exp4/M2_exp4_'+fnames1[i]+'_buoyancy.nc'
       else:
         path_file = path+fnames1[i]+'/Sigma_zstar/M2_exp4/M2_exp4_'+fnames1[i]+'.nc'
   ...
   for i in range(len(labels1)):
       if var in var_list:
         path_file = path+fnames1[i]+'/Sigma_zstar/M2_exp14/M2_exp14_'+fnames1[i]+'_buoyancy.nc'
       else:
         path_file = path+fnames1[i]+'/Sigma_zstar/M2_exp14/M2_exp14_'+fnames1[i]+'.nc'
       print 'path',path_file
       if n <=1:
          data = netCDF4.Dataset(path_file).variables[var][:]/1.0e12 # TW
       else:
