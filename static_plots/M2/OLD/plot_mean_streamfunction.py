...
def get_data(cs,h,depth,z,exp,im):
    ...
    temp=s.variables['temp'][-2::,:,:,:].mean(axis=0).mean(axis=2)
    salt=s.variables['salt'][-2::,:,:,:].mean(axis=0).mean(axis=2)
    ...
    psi1 = np.ma.masked_where(psi1==-1.0e32, psi1)
...
def driver(args):
   ...
   # read data
   psi_vals = np.linspace(-0.075,0.075,20)
   ...
   s=ax.contour(Y,-Z,rho-1000,[37.0,37.10,37.2],colors='k',lw=0.5)
