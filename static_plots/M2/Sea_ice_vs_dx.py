...
for j in range(len(dx)):
  ...
  for i in range(len(exps1)):
     path_to_file1 = path+'dx'+dx[j]+'/Sigma_zstar/'+exps1[i]+'/'+exps1[i]+'_'+dx[j]+'_buoyancy.nc'
     path_to_file2 = path+'dx'+dx[j]+'/Sigma_zstar/'+exps2[i]+'/'+exps2[i]+'_'+dx[j]+'_buoyancy.nc'
     ...
     print '--------------------------------- \n'
     data1 = netCDF4.Dataset(path_to_file1).variables['seaiceVolume'][:] # m^3
     data2 = netCDF4.Dataset(path_to_file2).variables['seaiceVolume'][:]
  ...
  if j == 2:
    ax.set_ylabel(r'Sea ice volume [m$^3$]', fontsize=20)
