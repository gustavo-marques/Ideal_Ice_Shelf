...
for j in range(len(dx)):
  ...
  f, ax = plt.subplots(len(variables), sharex=True, figsize=(8,3))
  for v in range(len(variables)):
      if v == 0:
        ax.set_title(r'$\Delta x$ = %s' %(dx2[j]), fontsize=18)
        txt1 = str('ax.plot(wind, %s_1, "-o", color=colors[0], linewidth=2, label="Melting on")' %(variables[v]))
        txt2 = str('ax.plot(wind, %s_2, "-*", color=colors[1], linewidth=2, label="Melting off")' %(variables[v]))
        exec(txt1); exec(txt2)
        ax.legend(loc='best',framealpha=0, fancybox=True)
      else:
        # plot
        txt = str('ax.plot(wind, %s_1, "-o", color=colors[0], linewidth=2)' %(variables[v]))
        exec(txt)
        txt = str('ax.plot(wind, %s_2, "-*", color=colors[1], linewidth=2)' %(variables[v]))
        exec(txt)

      # y label
      ax.set_ylabel('%s [W m$^{-2}$]' %(variables[v]))
  # total
  #txt1 = str("TOT_1 = %s_1[:] - %s_1[:]" %(variables[0], variables[1]))
  #txt2 = str("TOT_2 = %s_2[:] - %s_2[:]" %(variables[0], variables[1]))
  #exec(txt1); exec(txt2)
  # plot total
  ...
  ax.set_xlabel(r'$U_{asf}$ [m s$^{-1}$]',fontsize=18)
  ax.set_xlim(-5.5,5.5)
