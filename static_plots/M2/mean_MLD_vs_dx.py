...
for j in range(len(dx)):
  ...
  for i in range(len(exps1)):
     ...
     print '--------------------------------- \n'
     data1 = netCDF4.Dataset(path_to_file1).variables['MLD'][:]
     data2 = netCDF4.Dataset(path_to_file2).variables['MLD'][:]
  ...
  if j == 2:
    ax.set_ylabel(r'Domain-averaged BLD [m]', fontsize=20)
