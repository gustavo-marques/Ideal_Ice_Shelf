...
def get_data(exp):
    ...
    if tmp>24:
        v=s.variables['vh'][-24::,:,:,:].mean(axis=0).sum(axis=0)
        u=s.variables['uh'][-24::,:,:,:].mean(axis=0).sum(axis=0)
    else:
        v=s.variables['vh'][:,:,:,:].mean(axis=0).sum(axis=0)
        u=s.variables['uh'][:,:,:,:].mean(axis=0).sum(axis=0)
    s.close()
